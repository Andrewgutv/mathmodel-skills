...
def format_result_summary_block(summaries: dict[str, list[dict[str, str]]]) -> str:
    ...
    for question in sorted(k for k in summaries.keys() if k != "综合"):
...
def build_conclusion_sentence(question: str, rows: list[dict[str, str]]) -> str:
    if not rows:
        return f"{question}暂时没有可引用的结果。"
    first = rows[0]
    metric = first.get("指标", "").strip()
    value = first.get("数值", "").strip()
    note = first.get("说明", "").strip()
    return f"{question}当前得到的关键结果是：{metric} = {value}，可作为“{note}”的初步结论。"
...
def build_paper_sentence(question: str, rows: list[dict[str, str]]) -> str:
    ...
    value = first.get("数值", "").strip()
    note = first.get("说明", "").strip()
    return f"对于{question}，由计算结果可知，{metric}为{value}，这表明{note}。"
...
def build_defense_sentence(question: str, rows: list[dict[str, str]]) -> str:
    ...
    value = first.get("数值", "").strip()
    note = first.get("说明", "").strip()
    return f"{question}我们最终得到的核心结论是：{metric}等于{value}，它直接支持“{note}”这个判断。"
...
def build_figure_hint_string(question: str) -> str:
    return f"`图片/问题{question.replace('问题', '')}_配图说明.md`"
...
def replace_or_append_summary(path: Path, summary_block: str) -> None:
    ...
    if SECTION_MARKER in text:
        marker_line = SECTION_MARKER
        before, after = text.split(marker_line, 1)
        after = re.sub(
            r"^\s*## 自动回填结果摘要.*?(?=(\n# |\n## |\Z))",
            "",
            after,
...
def replace_between_markers(path: Path, start_marker: str, end_marker: str, block: str) -> None:
    ...
    if start_marker == QUESTION_RESULTS_START:
        text = re.sub(
            r"(?s)\n## 问题\d+结果回填.*?(?=(\n<!-- AUTO_QUESTION_RESULTS_START -->|\Z))",
            "\n",
            text,
        )
    if start_marker == DEFENSE_RESULTS_START:
        text = re.sub(
            r"(?s)\n## 问题\d+答辩结论.*?(?=(\n<!-- AUTO_DEFENSE_RESULTS_START -->|\Z))",
            "\n",
            text,
        )
