...
def get_paths() -> dict[str, Path]:
    ...
    return {
        ...
        "figure_stub": project_root / "图片" / f"图_{PROBLEM_TAG}_仿真结果说明.txt",
    }


def load_inputs(paths: dict[str, Path]) -> dict[str, str]:
    return {"data_source": str(paths["data_dir"]), "task_type": "simulation"}


def build_model(data: dict[str, str]) -> dict[str, str]:
    return {"model_family": "simulation", "note": "replace with ODE, Monte Carlo, or discrete simulation"}


def solve_model(model: dict[str, str], data: dict[str, str]) -> dict[str, str]:
    return {"simulation_metric": "0", "model_family": model["model_family"], "note": model["note"]}
...
def export_results(result: dict[str, str], paths: dict[str, Path]) -> None:
    ...
    with paths["result_csv"].open("w", encoding="utf-8", newline="") as f:
        ...
        writer.writerow(["simulation_metric", result["simulation_metric"], result["note"], Path(__file__).name])
