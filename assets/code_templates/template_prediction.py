...
def get_paths() -> dict[str, Path]:
    ...
    return {
        ...
        "figure_stub": project_root / "图片" / f"图_{PROBLEM_TAG}_预测结果说明.txt",
    }


def load_inputs(paths: dict[str, Path]) -> dict[str, str]:
    return {"data_source": str(paths["data_dir"]), "task_type": "prediction"}


def build_model(data: dict[str, str]) -> dict[str, str]:
    return {"model_family": "prediction", "note": "replace with regression or forecasting model"}
...
def export_results(result: dict[str, str], paths: dict[str, Path]) -> None:
    ...
    with paths["result_csv"].open("w", encoding="utf-8", newline="") as f:
        ...
        writer.writerow(["summary_metric", result["summary_metric"], result["note"], Path(__file__).name])
